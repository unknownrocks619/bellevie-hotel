...
<head>
    ...
    {{-- SEO Meta Tags --}}
    @php
        $seoTitle       = View::yieldContent('seo_title') ?: ($seoTitle ?? null) ?: \App\Models\Setting::get('hotel_name', 'Bellevie Hotel');
        $seoDescription = View::yieldContent('seo_description') ?: ($seoDescription ?? null) ?: \App\Models\Setting::get('hotel_description', '');
    ...
    @php
        ...
        $primaryColorDark = '#' . implode('', array_map(fn($c) => str_pad(dechex(max(0, hexdec($c) - 20)), 2, '0', STR_PAD_LEFT),
    ...
    <footer>
        <div class="container">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-3 mb-4">
                    <h6 class="text-uppercase fw-bold mb-3" style="color: var(--gold); font-size: 0.8rem; letter-spacing: 1px;">Contact</h6>
                    <p class="text-white-50 small mb-1">
                        <i class="bi bi-geo-alt me-2"></i>{{ $footerAddress }}, {{ $footerCity }}, {{ $footerCountry }}
                    </p>
                    <p class="text-white-50 small mb-1">
                        <i class="bi bi-telephone me-2"></i><a href="tel:{{ $footerPhone }}" style="color: inherit; text-decoration: none;">{{ $footerPhone }}</a>
                    </p>
                    <p class="text-white-50 small mb-1">
                        <i class="bi bi-envelope me-2"></i><a href="mailto:{{ $footerEmail }}" style="color: inherit; text-decoration: none;">{{ $footerEmail }}</a>
                    </p>
                </div>
                <div class="col-md-3 mb-4">
                    <h6 class="text-uppercase fw-bold mb-3" style="color: var(--gold); font-size: 0.8rem; letter-spacing: 1px;">Hours</h6>
                    <p class="text-white-50 small mb-1">
                        <i class="bi bi-box-arrow-in-right me-2"></i>Check-in: {{ \Carbon\Carbon::createFromFormat('H:i', $footerCheckIn)->format('g:i A') }}
                    </p>
                    <p class="text-white-50 small mb-1">
                        <i class="bi bi-box-arrow-right me-2"></i>Check-out: {{ \Carbon\Carbon::createFromFormat('H:i', $footerCheckOut)->format('g:i A') }}
                    </p>
                ...
                <div class="col-md-3 mb-4">
                    <h6 class="text-uppercase fw-bold mb-3" style="color: var(--gold); font-size: 0.8rem; letter-spacing: 1px;">Follow Us</h6>
                    <div class="d-flex gap-3 fs-5">
                        @if($footerFacebook && $footerFacebook !== '#')
                        <a href="{{ $footerFacebook }}" target="_blank" rel="noopener" class="text-white-50"><i class="bi bi-facebook"></i></a>
                        @endif
                        @if($footerInstagram && $footerInstagram !== '#')
                        <a href="{{ $footerInstagram }}" target="_blank" rel="noopener" class="text-white-50"><i class="bi bi-instagram"></i></a>
                        @endif
                        @if($footerTwitter && $footerTwitter !== '#')
                        <a href="{{ $footerTwitter }}" target="_blank" rel="noopener" class="text-white-50"><i class="bi bi-twitter-x"></i></a>
                        @endif
                    </div>

                    @if($footerPages->count() > 0)
                    <div class="mt-4">
                        <h6 class="text-uppercase fw-bold mb-2" style="color: var(--gold); font-size: 0.8rem; letter-spacing: 1px;">Information</h6>
                        @foreach($footerPages as $fPage)
                        <div class="mb-1">
                            <a href="{{ route('page.show', $fPage->slug) }}" class="text-white-50 small" style="text-decoration: none;">
                                <i class="bi bi-chevron-right me-1" style="font-size: 0.65rem;"></i>{{ $fPage->title }}
                            </a>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="row pt-3 border-top border-secondary">
                <div class="col-md-8">
                    <small class="text-white-50">&copy; {{ date('Y') }} {{ $footerHotelName }}. All rights reserved.</small>
                </div>
                <div class="col-md-4 text-md-end">
                    <small class="text-white-50">Powered by <span style="color: var(--gold);">Bellevie CMS</span></small>
                </div>
            </div>
        </div>
    </footer>
