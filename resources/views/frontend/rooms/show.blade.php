...
@section('content')
    ...
    @include('frontend.partials.page-hero', [
        'eyebrow' => strtoupper($room->roomType->name ?? 'Room'),
        'title' => $room->name,
        'subtitle' =>
            $room->show_price ?? true
                ? \App\Models\Setting::get('currency_symbol', '$') .
    ...
    <div class="container py-5">
        <div class="row mb-4">
            <div class="col-lg-8">
                <!-- Hero Image -->
                <img src="{{ $room->featuredImageUrl('https://via.placeholder.com/800x400') }}" alt="{{ $room->name }}"
                ...
                @elseif($room->gallery_images && count($room->gallery_images) > 0)
                    <div class="row g-2 mb-4">
                        @foreach ($room->gallery_images as $imageUrl)
                            <div class="col-4">
                                <img src="{{ $imageUrl }}" alt="{{ $room->name }}" class="img-fluid rounded"
                                    style="height: 150px; object-fit: cover; cursor: pointer;" data-bs-toggle="modal"
                                    data-bs-target="#galleryModal" onclick="updateGalleryImage('{{ $imageUrl }}')">
                            </div>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="mb-4">
                    ...
                    @if ($room->show_price ?? true)
                        <h5 style="color: #C9A227; margin: 0;">
                            {{ \App\Models\Setting::get('currency_symbol', '$') }}{{ number_format($room->price_per_night) }}<span
                                style="font-size: 0.8rem; color: #666;">/night</span></h5>
                    @else
                        <p style="font-size:.9rem;color:#aaa;font-style:italic;margin:0;">Price on request — contact us for
                            rates</p>
                    @endif
                </div>
                ...
                @if ($room->amenities && $room->amenities->count() > 0)
                    <div class="mb-4">
                        <h5 class="fs-4">Room Amenities</h5>
                        <div class="row">
                            @foreach ($room->amenities as $amenity)
                                <div class="col-md-6 mb-2">
                                    <div class="d-flex align-items-center">
                                        <i class="bi {{ $amenity->icon }}"
                                            style="font-size: 20px; color:#C9A227; margin-right: 10px;"></i>
                            ...
                            @foreach ($similarRooms as $similar)
                                <div class="col-md-6">
                                    <div class="card h-100 shadow-sm">
                                        <img src="{{ $similar->featuredImageUrl('https://via.placeholder.com/400x200') }}"
                                        ...
                                        <div class="card-body">
                                            ...
                                            @if ($similar->show_price)
                                                <p class="card-text mb-2"
                                                    style="color: #C9A227; font-weight: 600; margin-top: 0.5rem;">
                                                    {{ \App\Models\Setting::get('currency_symbol', '$') }}{{ number_format($similar->price_per_night, 2) }}<span
                                                        style="font-size: 0.85rem; color: #666;">/night</span></p>
                                            @endif
                                        ...
                                        <div class="card-footer bg-white border-top">
                                            <div class="d-flex gap-2">
                                                ...
                                                <a href="{{ route('booking.create', ['room' => $similar->id]) }}"
                                                    class="btn btn-sm"
                                                    style="background: #C9A227; color: white; border: none;">Book</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="card-body">
                        <form action="{{ route('booking.create') }}" method="GET">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="check_in" class="form-control datepicker"
                                    placeholder="Select date" required>
                            </div>

                            <div class="mb-3">
                                <label class="form-label" style="font-weight: 600;">Check-out Date</label>
                                <input type="text" name="check_out" class="form-control datepicker"
                                    placeholder="Select date" required>
                            </div>
                            ...
                            <button type="submit" class="btn w-100"
                                style="background:#C9A227; color:white; border:none; font-weight: 600; padding: 0.75rem;">Reserve
                                This Room</button>
                        </form>
                    </div>
                </div>
