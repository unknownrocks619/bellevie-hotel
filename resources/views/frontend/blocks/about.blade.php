@php
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        if ($val) $stats[] = ['value' => $val, 'label' => $lbl];
    }

    // ── Responsive min-height ─────────────────────────────────────────────────
    $presetMap = [
        'compact'=>'320px','standard'=>'520px','tall'=>'680px',
        'extratall'=>'820px','fullscreen'=>'100vh','almostfull'=>'90vh','threequarters'=>'75vh',
    ];
    $resolveH = function(array $bpCfg) use ($presetMap): string {
        if (($bpCfg['preset'] ?? '') === 'custom') return max(100,(int)($bpCfg['custom']??520)).'px';
...
@if($hasH)
<style>
    #{{ $blockId }} { min-height: {{ $hDesktop }}; display:flex; align-items:center; }
    @media (max-width:1199px) { #{{ $blockId }} { min-height: {{ $hLaptop }}; } }
    @media (max-width:991px)  { #{{ $blockId }} { min-height: {{ $hTablet }}; } }
    @media (max-width:575px)  { #{{ $blockId }} { min-height: {{ $hPhone  }}; } }
</style>
@endif

<section id="{{ $blockId }}" style="background:{{ $bg }};padding:80px 0;width:100%;">
    <div class="container">
        <div class="row align-items-center g-5 {{ $imgPos === 'right' ? 'flex-row-reverse' : '' }}">

            {{-- Image column --}}
            <div class="col-lg-5">
                @if($imgUrl)
                <div style="position:relative;border:3px solid #C9A227;padding:0.75rem;">
                    <img src="{{ $imgUrl }}" alt="{{ $config['title'] ?? '' }}"
                         class="img-fluid d-block w-100" style="display:block;">
                </div>
                @else
                <div style="height:360px;background:linear-gradient(135deg,#e0d4b0,#C9A22740);
            ...
            <div class="col-lg-7">
                ...
                <p style="color:#C9A227;font-weight:700;font-size:0.8rem;
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-4">
                        <div style="border-left:3px solid #C9A227;padding-left:14px;">
                            <div style="font-size:1.8rem;font-weight:700;color:#C9A227;
