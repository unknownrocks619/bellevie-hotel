            ...
            @foreach ($rooms as $room)
                <div class="{{ $colClass }}">
                    <div class="card h-100"
                        style="border:none;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,.08);transition:transform .2s,box-shadow .2s;"
                        ...
                        <div style="position:relative;overflow:hidden;">
                            ...
                            @if ($room->is_featured)
                                <span
                                    style="position:absolute;top:12px;right:12px;background:#C9A227;color:#fff;font-size:0.7rem;padding:4px 10px;border-radius:20px;font-weight:600;">
                                    Featured
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="card-body d-flex flex-column">
                            <div class="mb-1">
                                <span
                                    style="font-size:0.72rem;color:#C9A227;font-weight:600;text-transform:uppercase;letter-spacing:.08em;">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-3 mt-auto">
                                @if ($room->bed_type)
                                    <small class="text-muted"><i
                                            class="bi bi-moon me-1"></i>{{ $room->bed_type }}</small>
                                @endif
                                @if ($room->max_adults)
                                    <small class="text-muted"><i
                                            class="bi bi-people me-1"></i>{{ $room->max_adults + ($room->max_children ?? 0) }}</small>
                                @endif
                                @if ($room->size_sqft)
                                    <small class="text-muted"><i
                                            class="bi bi-arrows-fullscreen me-1"></i>{{ $room->size_sqft }} ft²</small>
                                @endif
                            </div>

                            @if ($config['showPrice'] ?? true)
                                <div class="mb-3"
                                    style="color:#C9A227;font-weight:700;font-size:1.1rem;font-family:'Playfair Display',serif;">
                                    {{ $symbol }}{{ number_format($room->price_per_night) }}
                                    <small
                                        style="color:#999;font-weight:400;font-size:0.8rem;font-family:inherit;">/night</small>
                                </div>
                            @endif
                        </div>
                        ...
                        @if (!empty($btnText))
                            <div class="card-footer bg-white border-0 pt-0 pb-3 px-3">
                                <a href="{{ $btnUrl ?: route('rooms.show', $room) }}" class="btn w-100"
